...
## 12. Каталог зафіксованих точок відновлення

До реального restore/контейнерного етапу додано явну модель restore point:

- у каталог потрапляють лише `verified` manifests, для яких існують усі
  записані artifacts і збігаються їхні розміри;
- каталог фільтрується за server/group і показує дату, installer ref/commit,
  склад, загальний розмір та snapshot SHA;
- користувач вибирає конкретну точку для поточного плану;
- план зберігає `baselineRestorePoint.id`, `createdAt` і стабільний
  `snapshotSha256`;
- новий backup поточного плану автоматично стає його зафіксованою точкою;
- нові batch transactions окремо зберігають `baselineSource` і `restorePoint`,
  тому попереднє неоднозначне поле `backupPlanId` більше не є єдиним джерелом
  походження;
- створення новішого backup не підміняє вже вибрану точку існуючого плану.

Каталог поки є вибором і аудитом джерела. Full archive restore залишається
окремою операцією; batch rollback конфігів, як і раніше, використовує точні T2
snapshots конкретного apply.

## 13. Drift-safe rollback gate

Перед першим реальним batch rollback виявлено, що helper v3 перевіряв SHA T2,
але не перевіряв поточний SHA цільового файла. Це створювало ризик затерти
ручну зміну, зроблену після StandWatch apply.

У review `0.0.0-review.20260924.4` введено helper v4 і обов'язковий rollback
preflight:

- якщо live SHA дорівнює `before`, файл уже відновлено і повторний запис не
  виконується;
- якщо live SHA дорівнює `target` конкретної транзакції, дозволяється atomic
  restore її T2;
- будь-який третій SHA вважається drift і блокує rollback без зміни файла;
- batch спочатку перевіряє всі дочірні транзакції й лише потім переходить у
  `rollback-in-progress`, тому preflight-помилка не створює partial rollback;
- helper v3 не використовується для rollback: UI пропонує одноразово встановити
  v4 тим самим контрольованим sudo-flow.

Контейнери під час цієї операції, як і раніше, не перезапускаються.

## 14. Container rollout scope — review.20260924.5

Перед реалізацією remote execution додано окрему чисту модель контейнерної
операції та її dry-run у поточному installer-плані. Прийнята політика:

- зміна хоча б одного service image/tag розширює scope до **всієї прив'язаної
  Compose-групи**; усі її контейнери мають бути force-recreate однією повною
  Compose-моделлю, щоб не пропустити залежності;
- зміна `home/*.yml|yaml` також означає recreate всієї групи;
- зміна bind-mounted файла під `volumes/config/` без зміни image вимагає
  restart усіх контейнерів, які реально монтують цей файл або батьківський
  каталог;
- відповідність config → container визначається за live mount topology, а не
  за схожістю імен;
- змінений config без знайденого mount блокує операцію;
- зміна `scripts/*.sh` сама по собі не запускає скрипт і не перезапускає
  контейнери;
- контейнери інших Compose-груп на тому самому Docker host явно виключаються.

Preflight тепер фіксує поточний Docker image ID кожного контейнера. Для
повногрупового rollout відсутній image ID є блокером: перед pull старі images
мають отримати захисні rollback refs. Запланований rollback повертає файловий
T2, попередні image refs, recreate всієї групи та повторні health/application
checks.

Remote execution у цій версії навмисно вимкнений. UI показує лише зафіксовані
scope, контейнери й блокери. Безпосередньо перед майбутнім виконанням inventory,
mounts, live tags/SHA та image IDs мають бути перевірені повторно.
